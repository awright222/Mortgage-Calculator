...
const TaxesInsuranceCalculator = ({
  ...
  const calculateTaxesAndInsurance = () => {
    ...
    const calculatedTaxes = (homeValue * (taxRateNum / 100)) / 12; // Monthly property tax
    const calculatedInsurance = insuranceCostNum / 12; // Monthly insurance cost
    ...
    const newMonthlyPayment = monthlyPayment + calculatedEscrow - escrow; // Adjust for previous escrow
    const newTotalRepayment =
      totalRepayment + calculatedEscrow * 12 * termLength - escrow * 12 * termLength; // Adjust for previous escrow
  ...
  return (
    <div className="taxes-insurance-calculator">
      ...
      <button
        className="w-full bg-[#FFB703] text-white rounded-lg py-2 font-semibold hover:bg-green-600 mt-4"
